...
namespace MMI.UIControl
{
    public partial class B_AGauge : UserControl
    {
        ...
        [System.ComponentModel.Browsable(true),
        System.ComponentModel.Category("AGauge"),
        System.ComponentModel.Description("范围推荐速度")]
        public float RecommedSpeed_
        {
            get
            {
                if (_recommedSpeed_ > 110)
                {
                    _recommedSpeed_ = 110;
                }
                else if (_recommedSpeed_ < 0)
                {
                    _recommedSpeed_ = 0;
                }
                return _recommedSpeed_;
            }
            set
            {
                if (value > 110)
        ...
        private void B_picBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            SolidBrush recommendBrush_ = new SolidBrush(Color.GreenYellow);
            ...
            #region//绘制推荐速度范围箭头
            PointF[] recommendArrowArr_ = new PointF[3];
            recommendArrowArr_[0].Y = _recommedSpeed_ / 120 * 360 + _stepAngle;
            recommendArrowArr_[0].X = 130;
            recommendArrowArr_[1].X = recommendArrowArr_[2].X = 145;
            recommendArrowArr_[1].Y = recommendArrowArr_[0].Y - 3;
            recommendArrowArr_[2].Y = recommendArrowArr_[0].Y + 3;
            for (int i = 0; i < 3; i++)
            {
                recommendArrowArr_[i] = ORtoXY(recommendArrowArr_[i]);
            }
            g.FillPolygon(recommendBrush_, recommendArrowArr_);
            #endregion
